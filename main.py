...
if __name__ == '__main__':
    ...
    if (LSTM):
        print("running LSTM")
        X, y, v_size, char_to_token, token_to_char = get_LSTM_data("data/shakespeare.txt", True, 2)
        #model = train_LSTM(X, y, v_size)
        model1 = train_LSTM(X, y, v_size, 0.25)
        model1.save('lstm_t25_2.model')

        model2 = train_LSTM(X, y, v_size, 0.75)
        model2.save('lstm_t75_2.model')

        model3 = train_LSTM(X, y, v_size, 1.50)
        model3.save('lstm_t150_2.model')

        #model = models.load_model('m1.model')
        print(generate_seq(model1, char_to_token, token_to_char, "shall i compare thee to a summer's day?\n"))
